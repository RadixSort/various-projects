...
WordToLengthMap get_words_by_length(std::set<std::string> const &words) {
  ...
  for (std::set<std::string>::const_iterator i = words.begin();
       i != words.end();
       i++) {
    std::string word = *i;
    std::set<std::string> wordset;
    std::pair<int, std::set<std::string>> wordpair;
    if (word.length() != 0) {
      if (words_by_length.count(word.length()) < 1) {
        wordset.clear();
        wordset.insert(word);
        wordpair.first = word.length();
        wordpair.second = wordset;
        words_by_length.insert(wordpair);
      } else {
        words_by_length[word.length()].insert(word);
      }
    }
  }
