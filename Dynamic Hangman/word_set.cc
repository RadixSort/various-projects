...
WordSet WordSet::generate_new_wordset(char guess) const {
  ...
  if (wordsWithGuess.empty()) return WordSet(pattern_, words_);
...
std::vector<std::string> WordSet::find_matching_words(char guess) const {
  ...
  for (std::set<std::string>::iterator i = words_.begin();
       i != words_.end();
       i++) {
    std::string word = *i;
    if (word.find(guess) != -1) matchingWords.push_back(word);
  }
...
std::string WordSet::extract_pattern(std::string word, char guess) const {
  ...
  std::string oldPattern = pattern_;

  for (int i = 0; i < word.size(); i++) {
      if (word[i] == guess)
        newPattern += word[i];
      else
        newPattern += oldPattern[i];
    }
...
std::vector<WordSet> WordSet::partition(char guess) const {
  ...
  std::map<std::string, std::set<std::string>> pMap;

  for (std::string word : words_) {
    std::string pattern = extract_pattern(word, guess);
    if (pMap.count(pattern) < 1) {
      set.clear();
      set.insert(word);
      pair.first = pattern;
      pair.second = set;
      ...
      pMap[pattern].insert(word);
    }
  }

  if (words_.size() != 0) {
    for (std::map<std::string,
        std::set<std::string>>::const_iterator i = pMap.cbegin(); i
        != pMap.cend(); i++)
      sets.push_back(WordSet((*i).first, (*i).second));
  }
  return sets;
}
