...
#define CSCR   0x31  /*Check Sum Control Register */
#define CSCR_set 0x07  /*Enable IP, TCP, UDP Checksum generation*/
#define RCSCSR  0x32  /*Receive Check Sum STatus register*/
#define RCSCSR_set 0x00
#define NCR_set    0x00
#define TCR_set    0x00
...
#define PHY_mode   0x3100  /* set PHY media mode: Auto negotiation
 ...
 #define TCSCR  0x31  /* Transmit Checksum Control Register REG. 31H */
#define TCSCR_set  0x01  /* Bit[0]=1 means IP Checksum is enabled 
                            Bit[1]=1 means TCP Checksum is enabled
                            Bit[2]=1 means UDP Checksum is enabled
                            Bit[7 to 3] are reserved*/
                            
  /*              
  Bit[1] Receive CheckSum Checking Enable. When set, the checksum status (bit 7~2) will be stored in packet’s first byte(bit
7~2) of status header respectively.

Bit[0] mean Discard CheckSum Error Packet, When set, if IP/TCP/UDP checksum field is error, this packet will be discarded.
*/                
